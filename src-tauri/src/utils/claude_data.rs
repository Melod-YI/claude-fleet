...
pub fn get_running_sessions_list() -> Result<Vec<ClaudeSession>, String> {
    info!("[get_running_sessions_list] 开始获取运行中 session 列表");
    let sessions_dir = get_sessions_dir();
    debug!("[get_running_sessions_list] sessions 目录: {}", sessions_dir.display());

    if !sessions_dir.exists() {
        warn!("[get_running_sessions_list] sessions 目录不存在，返回空列表");
        return Ok(Vec::new());
    }
    ...
    let entries = fs::read_dir(&sessions_dir)
        .map_err(|e| {
            error!("[get_running_sessions_list] 读取 sessions 目录失败: {}", e);
            format!("读取 sessions 目录失败: {}", e)
        })?;
    ...
    for entry in entries {
        let file_path = entry
            .map_err(|e| {
                warn!("[get_running_sessions_list] 读取条目失败: {}", e);
                format!("读取条目失败: {}", e)
            })?
            .path();

        if file_path.extension().and_then(|s| s.to_str()) != Some("json") {
            debug!("[get_running_sessions_list] 跳过非 json 文件: {}", file_path.display());
            continue;
        }

        file_count += 1;
        debug!("[get_running_sessions_list] 处理文件 #{}: {}", file_count, file_path.display());

        let content = fs::read_to_string(&file_path)
            .map_err(|e| {
                warn!("[get_running_sessions_list] 读取文件 {} 失败: {}", file_path.display(), e);
                format!("读取文件 {} 失败: {}", file_path.display(), e)
            })?;

        if let Ok(metadata) = serde_json::from_str::<RunningSessionMetadata>(&content) {
            debug!("[get_running_sessions_list] 解析成功: pid={}, session_id={}, status={}",
                   metadata.pid, metadata.session_id, metadata.status);

            // 检查进程是否在运行
            let is_running = is_process_running(metadata.pid);
            if !is_running {
                exited_count += 1;
                warn!("[get_running_sessions_list] 进程 {} 已退出，跳过 session {}",
                      metadata.pid, metadata.session_id);
                continue;
            }
            ...
            let status = if metadata.status == "idle" {
                waiting_count += 1;
                debug!("[get_running_sessions_list] session {} 状态为 idle -> waiting_input", metadata.session_id);
                "waiting_input".to_string()
            } else {
                running_count += 1;
                debug!("[get_running_sessions_list] session {} 状态为 {} -> running", metadata.session_id, metadata.status);
                "running".to_string()
            };
            ...
            warn!("[get_running_sessions_list] 解析失败: {}", file_path.display());
        }
    }

    info!("[get_running_sessions_list] 统计: 文件数={}, 运行中={}, 等待输入={}, 已退出={}",
          file_count, running_count, waiting_count, exited_count);

    // 按状态排序：waiting_input 优先
    debug!("[get_running_sessions_list] 排序：waiting_input 优先");
    ...
    info!("[get_running_sessions_list] 完成，返回 {} 个 session", sessions.len());
    Ok(sessions)
}
