...
- 启动终端窗口：直接运行目标程序（cmd.exe / powershell.exe / wezterm），使用 `DETACHED_PROCESS` 脱离父进程
- 执行后台命令（tasklist、wmic 等）：通过 `cmd.exe /C` 配合 `CREATE_NO_WINDOW` 隐藏窗口

**Windows 外部命令执行经验总结：**

Windows GUI 应用（如 Tauri）可能不继承终端的完整 PATH 环境变量，且直接启动控制台程序会弹出窗口。项目中区分两种场景：

**场景一：启动终端窗口（新建/恢复 session）**
直接运行目标终端程序，使用 `DETACHED_PROCESS` 脱离父进程，无需 `cmd.exe /c start` 包装：
```rust
...
Command::new("cmd.exe")
    .args(["/d", &cwd, "/K", "claude --permission-mode bypassPermissions"])
    .creation_flags(DETACHED_PROCESS)
    .spawn()
```

**场景二：执行后台命令（tasklist、wmic、code 等）**
通过 `cmd.exe /C` 配合 `CREATE_NO_WINDOW` 隐藏窗口：
```rust
...
**关键要点：**
- `DETACHED_PROCESS = 0x00000008` 使子进程脱离父进程，终端窗口独立运行
- `CREATE_NO_WINDOW = 0x08000000` 完全隐藏进程窗口，适用于后台命令
...
- 启动终端窗口直接运行目标程序（DETACHED_PROCESS），后台命令通过 `cmd.exe /C` + `CREATE_NO_WINDOW` 执行